# aboutme.md — Profil de marque Voyages21

Fichier central lu par Claude avant toute production de contenu.

## Identité
- **Marque** : Voyages21 — agence de voyages sur mesure au Maroc depuis 2000
- **Fondateur** : Karim Zoubdane (visage de la marque, 25 ans d'expérience)
- **Site** : https://voyages21.vercel.app/ (prod) — www.voyages21.com
- **Signature** : « De l'aventure intime aux projets d'envergure, Voyages 21 signe le voyage. »

## Codes visuels (repris du design system du site)
- Vert forêt `#1B3A28` / vert profond `#152E1F` — fonds, élégance
- Or `#C8A440` — accents, séparateurs, prestige
- Crème `#F5F0E8` — textes sur fond sombre, douceur
- Typo titres : Playfair Display (italique autorisé) — typo corps : DM Sans
- Logo : `public/logo-voyages21.png` (importé dans Higgsfield,
  media_id `bf6f77a5-ddc3-4138-9ba5-66e97a4f7a03`)

## Offre (source : pages /voyages du site)
- Maroc : circuits impériaux, Grand Discovery, Dakhla, désert (Zagora…)
- International : Europe (Italie, Grèce, Espagne-Portugal, Las Palmas),
  Turquie, Asie (Chine, Vietnam), USA (Ouest, Rocheuses)
- Religieux : Omra (départs réguliers, formules avec Istanbul)

## Règles de contenu (non négociables)
1. Jamais de faux témoignages clients ni de faux influenceurs générés par IA.
2. Les visuels IA sont **conceptuels/inspirationnels** ; les lieux réels sont
   montrés avec de vraies photos/vidéos (retravaillées par IA si besoin).
3. Toute promesse (prix, dates, programme) doit venir des fiches voyages du site.
4. Ton : chaleureux, expert, premium accessible. Français d'abord ; arabe
   (darija) possible pour les réseaux sociaux marocains.

## À compléter (interview fondateur — réponses de Karim)
- **Q1. Client idéal** ✅ (12/06/2026) — Deux activités distinctes :
  - **Outgoing** (Marocains vers l'étranger, via www.voyages21.com) : tous
    profils — familles marocaines, MRE, couples, retraités, entreprises,
    pèlerins Omra.
  - **Incoming** (étrangers vers le Maroc, via un 2e site en construction) :
    ciblage par nationalité (Anglais, Russes, Allemands, etc.) — familles,
    couples, associations, entreprises (voyages du personnel / budgets
    sociaux). Tout profil souhaitant découvrir le Maroc.
  - **Budget publicitaire** (Meta Ads + Google Ads) : à fixer ensemble après
    recherches (audiences, mots-clés, CPC par marché). Accès aux comptes pub
    par délégation officielle uniquement (jamais de partage de mot de passe).
- **Q2. Plateformes prioritaires** ✅ (12/06/2026) — Comptes existants :
  - Instagram : **@voyages21maroc** (Meta Verified)
  - Facebook : **« Voyages Maroc »** (3,5 K amis, Marrakech — profil à
    faire évoluer vers une Page pro pour les ads)
  - TikTok : **@voyages.maroc** (73 abonnés, bio à créer)
  - WhatsApp Business : 0661247049 (canal de conversion : statuts + catalogue)
  - YouTube : identifiants à communiquer par Karim (➡️ À LUI RAPPELER)
  - Snapchat, LinkedIn : pas encore créés
  - **Priorités recommandées par Claude** : P1 Instagram + Facebook (outgoing
    Maroc/Omra), P2 TikTok (à rebrander), P3 YouTube Shorts (incoming
    international/SEO), LinkedIn seulement en phase B2B (entreprises/CE),
    Snapchat en phase 3 si budget ads dédié.
  - **Google Business Profile** : oui, intégré au calendrier — Claude prépare
    les posts/offres prêts à coller (publication manuelle par Karim au début).
- **Q3. Objectif n°1 (juin→sept. 2026)** ✅ (12/06/2026) — Vendre les voyages
  OUTGOING vers l'étranger via www.voyages21.com (PAS le Maroc en priorité).
  Ordre de priorité des destinations :
  1. **Égypte** — PRIORITÉ ABSOLUE : stock de billets d'avion à écouler (urgence).
  2. **Turquie** (Istanbul-Antalya, départs été déjà programmés).
  3. Autres destinations programmées ensuite.
  Règle : Claude demande TOUJOURS à Karim quels voyages prioriser avant
  de produire (ne jamais supposer le programme/les dates/les prix —
  les tirer des fiches du site).
- **Q3. Objectif n°1 des 3 prochains mois** : (notoriété ? leads Omra ? été 2026 ?)
- **Q4. Rythme de publication souhaité et temps de validation dispo/semaine** :
- **Q5. Concurrents/comptes de référence à surveiller** : (3-5 comptes IG/TikTok)
